...
PATH_MODEL_ULTIMATE = config.models_subdir / "ridge.pkl"
PATH_FEATURE_IMPORTANCE_MODEL_ULTIMATE = (
    config.models_subdir / "feature_importance_ridge.csv"
)
...
if not config_ridge.model_lambda:
    ...
    idx_model_best = np.argwhere(tune_grid["lambda"] <= config_ridge.model_lambda)[0][0]
...
feature_importance = pd.DataFrame(
    {"feature": X.columns, "score": np.abs(model.coef_), "coef": model.coef_}
